...
{
    /// <summary>
    ///     Marks the <see cref="Type"/> to be read by the specified <see cref="KaiHeiLa.Commands.TypeReader"/>.
    /// </summary>
    /// <remarks>
    ///     This attribute will override the <see cref="KaiHeiLa.Commands.TypeReader"/> to be used when parsing for the
    ///     desired type in the command. This is useful when one wishes to use a particular 
    ///     <see cref="KaiHeiLa.Commands.TypeReader"/> without affecting other commands that are using the same target
    ///     type.
    ///     <note type="warning">
    ///         If the given type reader does not inherit from <see cref="KaiHeiLa.Commands.TypeReader"/>, an 
    ///         <see cref="ArgumentException"/> will be thrown.
    ///     </note>
    /// </remarks>
    /// <example>
    ///     In this example, the <see cref="TimeSpan"/> will be read by a custom 
    ///     <see cref="KaiHeiLa.Commands.TypeReader"/>, <c>FriendlyTimeSpanTypeReader</c>, instead of the 
    ///     <see cref="TimeSpanTypeReader"/> shipped by KaiHeiLa.Net.
    ///     <code language="cs">
    ///     [Command("time")]
