...
/// <summary>
///     表示一个用于 <see cref="T:Kook.Webhook.HttpListener.KookHttpListenerWebhookClient"/> 的配置类。
/// </summary>
/// <remarks>
///     此配置基于 <see cref="T:Kook.Webhook.KookWebhookConfig"/>，在与 Webhook 有关的配置的基础上，定义了有关 HTTP 监听器的配置。
/// </remarks>
public class KookHttpListenerWebhookConfig : KookWebhookConfig
{
    ...
    /// </summary>
    /// <remarks>
    ///     设置为与 <see cref="F:System.Threading.Timeout.InfiniteTimeSpan"/>
    ///     相等的值表示客户端将在保持应用程序运行的情况下不重新启动；设置为其它任何负值将导致客户端在 HTTP 监听器关闭后退出应用程序；设置为
    ///     <see cref="F:System.TimeSpan.Zero"/> 将导致客户端在 HTTP
