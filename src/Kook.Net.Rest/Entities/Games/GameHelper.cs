...
{
    public static async Task DeleteAsync(IGame game, BaseKookClient client, RequestOptions? options) =>
        await client.ApiClient.DeleteGameAsync(game.Id, options).ConfigureAwait(false);

    public static async Task<RestGame> ModifyAsync(IGame game, BaseKookClient client,
        Action<GameProperties> func, RequestOptions? options)
    {
