...
internal class KookRestApiClient : IDisposable
{
    ...
    public async Task<IReadOnlyCollection<RichGuild>> ListGuildsAsync(RequestOptions options = null)
    {
        ...
        return await SendAsync<IReadOnlyCollection<RichGuild>>(HttpMethod.Get,
            () => $"guild/index", ids, clientBucket: ClientBucketType.SendEdit, options: options).ConfigureAwait(false);
    }

    public IAsyncEnumerable<IReadOnlyCollection<Guild>> GetGuildsAsync(int limit = KookConfig.MaxItemsPerBatchByDefault, int fromPage = 1, RequestOptions options = null)
    {
        options = RequestOptions.CreateOrClone(options);
        ...
        int limit = KookConfig.MaxUsersPerBatch, int fromPage = 1, RequestOptions options = null)
    {
        ...
        if (func is not null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(properties.SearchName))
                extendedQuery += $"&search={Uri.EscapeDataString(properties.SearchName)}";
            if (properties.RoleId.HasValue)
                extendedQuery += $"&role_id={properties.RoleId.Value}";
            ...
            if (properties.SortedByActiveTime.HasValue)
                extendedQuery += $"&active_time={(int)properties.SortedByActiveTime.Value}";
            if (properties.SortedByJoinTime.HasValue)
                extendedQuery += $"&joined_at={(int)properties.SortedByJoinTime.Value}";
        }
        var ids = new BucketIds(guildId: guildId);
    ...
    public async Task ModifyGuildMemberNicknameAsync(ModifyGuildMemberNicknameParams args, RequestOptions options = null)
    {
        ...
        if (args.Nickname?.Length > KookConfig.MaxNicknameSize)
            throw new ArgumentException(message: $"Nickname is too long, length must be less or equal to {KookConfig.MaxNicknameSize}.", paramName: nameof(args.Nickname));
        if (args.Nickname?.Length < KookConfig.MinNicknameSize)
            throw new ArgumentException(message: $"Nickname is too short, length must be more or equal to {KookConfig.MinNicknameSize}.", paramName: nameof(args.Nickname));
    ...
    public IAsyncEnumerable<IReadOnlyCollection<Channel>> GetGuildChannelsAsync(ulong guildId, int limit = KookConfig.MaxItemsPerBatchByDefault, int fromPage = 1, RequestOptions options = null)
    {
        Preconditions.NotEqual(guildId, 0, nameof(guildId));
        options = RequestOptions.CreateOrClone(options);
    ...
    public async Task<CreateMessageResponse> CreateMessageAsync(CreateMessageParams args, RequestOptions options = null)
    {
        ...
        if (args.Content?.Length > KookConfig.MaxMessageSize)
            throw new ArgumentException(message: $"Message content is too long, length must be less or equal to {KookConfig.MaxMessageSize}.", paramName: nameof(args.Content));
        options = RequestOptions.CreateOrClone(options);
    ...
    public IAsyncEnumerable<IReadOnlyCollection<UserChat>> GetUserChatsAsync(int limit = KookConfig.MaxItemsPerBatchByDefault, int fromPage = 1, RequestOptions options = null)
    {
        options = RequestOptions.CreateOrClone(options);
    ...
    public IAsyncEnumerable<IReadOnlyCollection<Role>> GetGuildRolesAsync(ulong guildId, int limit = KookConfig.MaxItemsPerBatchByDefault, int fromPage = 1, RequestOptions options = null)
    {
        Preconditions.NotEqual(guildId, 0, nameof(guildId));
        options = RequestOptions.CreateOrClone(options);
    ...
    public IAsyncEnumerable<IReadOnlyCollection<Emoji>> GetGuildEmotesAsync(ulong guildId, int limit = KookConfig.MaxItemsPerBatchByDefault, int fromPage = 1, RequestOptions options = null)
    {
        Preconditions.NotEqual(guildId, 0, nameof(guildId));
        options = RequestOptions.CreateOrClone(options);
    ...
    public IAsyncEnumerable<IReadOnlyCollection<Invite>> GetGuildInvitesAsync(ulong? guildId = null, ulong? channelId = null, int limit = KookConfig.MaxItemsPerBatchByDefault, int fromPage = 1, RequestOptions options = null)
    {
