...
internal class SocketGlobalUser : SocketUser
{
    ...
    public SocketGlobalUser(KookGatewayClient kook, ulong id)
        : base(kook, id)
    {
    }

    internal static SocketGlobalUser Create(KookGatewayClient kook, ClientState state, Model model)
    {
    ...
    internal void RemoveRef(KookGatewayClient kook)
    {
        lock (_lockObj)
        {
            if (--_references <= 0) kook.RemoveUser(Id);
        }
    }
