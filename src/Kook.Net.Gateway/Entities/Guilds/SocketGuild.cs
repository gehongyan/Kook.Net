...
public class SocketGuild : SocketEntity<ulong>, IGuild, IDisposable, IUpdateable
{
    ...
    ///     </note>
    ///     <note type="warning">
    ///         Only when <see cref="KookGatewayConfig.AlwaysDownloadUsers"/> is set to <c>true</c>
    ...
    ///     <note type="warning">
    ///         <para>
    ///             Only when <see cref="KookGatewayConfig.AlwaysDownloadBoostSubscriptions"/> is set to <c>true</c>
    ...
    ///             <see cref="KookGatewayClient.DownloadBoostSubscriptionsAsync"/> upon a <see cref="KookGatewayClient"/> to
    ...
    /// <seealso cref="KookGatewayClient.DownloadBoostSubscriptionsAsync"/>
    /// <seealso cref="KookGatewayClient.AlwaysDownloadBoostSubscriptions"/>
    ...
    ///         </para>
    ///         <para>
    ///             Otherwise, you may need to enable <see cref="KookGatewayConfig.AlwaysDownloadUsers"/> to fetch
    ...
    /// <seealso cref="KookGatewayClient.AlwaysDownloadUsers"/>
    /// <seealso cref="KookGatewayClient.DownloadUsersAsync"/>
    ...
    internal SocketGuild(KookGatewayClient kook, ulong id) : base(kook, id)
    {
        _audioLock = new SemaphoreSlim(1, 1);
    }

    internal static SocketGuild Create(KookGatewayClient client, ClientState state, Model model)
    {
    ...
    internal static SocketGuild Create(KookGatewayClient client, ClientState state, RichModel model)
    {
        SocketGuild entity = new(client, model.Id);
        entity.Update(state, model);
        return entity;
    ...
    /// </summary>
    /// <returns>
    ///     A string that resolves to <see cref="Name"/>.
    /// </returns>
    ...
    internal async Task<IAudioClient> ConnectAudioAsync(ulong channelId, /*bool selfDeaf, bool selfMute, */bool external, bool disconnect)
    {
        ...
        try
        {
            ...
            {
                var audioClient = new AudioClient(this, Kook.GetAudioId(), channelId);
    ...
    {
        SocketGuildUser selfUser = GetVoiceChannel(channelId).Users
            .SingleOrDefault(x => x.Id == Kook.CurrentUser.Id);
        if (selfUser is null) return;
        if (selfDeaf) await selfUser.DeafenAsync();
        else await selfUser.UndeafenAsync();
        if (selfMute) await selfUser.MuteAsync();
        else await selfUser.UnmuteAsync();
    }
