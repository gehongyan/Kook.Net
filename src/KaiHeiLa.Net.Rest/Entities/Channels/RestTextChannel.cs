...
public class RestTextChannel : RestGuildChannel, IRestMessageChannel, ITextChannel
{
    ...
    // {
    //     CreateAssetResponse createAssetResponse = await KaiHeiLa.ApiClient.CreateAssetAsync(new CreateAssetParams {File = File.OpenRead(path), FileName = fileName}, fileName, options);
