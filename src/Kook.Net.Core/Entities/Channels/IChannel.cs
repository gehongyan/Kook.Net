...
public interface IChannel : IEntity<ulong>
{
    ...
    ///     <c>50</c>，则请求将被拆分为 60 个单独请求，因此异步枚举器会异步枚举返回 60 个响应。<see cref="AsyncEnumerableExtensions.FlattenAsync{T}"/>
    ...
    ///     获取此频道中的用户。
    /// </summary>
    /// <param name="id"> 要获取的用户的 ID，例如 2810246202。 </param>
    /// <param name="mode"> 指示当前方法是否应该仅从缓存中获取结果，还是可以通过 API 请求获取数据。 </param>
    /// <param name="options"> 发送请求时要使用的选项。 </param>
    /// <returns>
    ///     一个表示异步获取操作的任务。任务结果包含表示找到的用户的用户对象；如果没有找到，则返回 <c>null</c>。
    /// </returns>
