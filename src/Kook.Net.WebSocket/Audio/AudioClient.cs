...
internal partial class AudioClient : IAudioClient
{
    ...
    public KookVoiceAPIClient ApiClient { get; private set; }
    ...
    private async Task OnConnectingAsync()
    {
        ...
        await ApiClient.ConnectAsync("wss://" + _url/* + "?v=" + KookConfig.VoiceAPIVersion*/).ConfigureAwait(false);
    ...
    private async Task ProcessMessageAsync(uint id, bool okay, object payload)
    {
        ...
        try
        {
            switch (type)
            {
                ...
                //         _ssrc = data.SSRC;
                //
                //         if (!data.Modes.Contains(KookVoiceAPIClient.Mode))
                //             throw new InvalidOperationException($"Kook does not support {KookVoiceAPIClient.Mode}");
                //
                //         ApiClient.SetUdpEndpoint(data.Ip, data.Port);
                ...
                //
                //         if (data.Mode != KookVoiceAPIClient.Mode)
                //             throw new InvalidOperationException($"Kook selected an unexpected mode: {data.Mode}");
                //
    ...
    //     // TODO: Clean this up when Kook's session patch is live
    //     try
    //     {
