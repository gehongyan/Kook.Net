...
public abstract class SocketMessage : SocketEntity<Guid>, IMessage, IUpdateable
{
    ...
    /// <inheritdoc />
    public IReadOnlyDictionary<IEmote, ReactionMetadata> Reactions => _reactions.GroupBy(r => r.Emote).ToDictionary(x => x.Key, x => new ReactionMetadata { ReactionCount = x.Count(), IsMe = x.Any(y => y.UserId == Kook.CurrentUser.Id) });
    ...
    internal static SocketMessage Create(KookSocketClient kook, ClientState state, SocketUser author, ISocketMessageChannel channel, GatewayGroupMessageExtraData model, GatewayEvent gatewayEvent)
    {
    ...
    internal static SocketMessage Create(KookSocketClient kook, ClientState state, SocketUser author, ISocketMessageChannel channel, GatewayPersonMessageExtraData model, GatewayEvent gatewayEvent)
    {
        if (model.Author.Id == KookConfig.SystemMessageAuthorID)
            return SocketSystemMessage.Create(kook, state, author, channel, model, gatewayEvent);
        else
    ...
    internal static SocketMessage Create(KookSocketClient kook, ClientState state, SocketUser author, ISocketMessageChannel channel, API.Message model)
    {
        if (model is null)
            return null;
        if (model.Author.Id == KookConfig.SystemMessageAuthorID)
    ...
    internal static SocketMessage Create(KookSocketClient kook, ClientState state, SocketUser author, ISocketMessageChannel channel, API.DirectMessage model)
    {
        if (model is null)
            return null;
