...
internal class ClientState
{
    ...
    public ClientState(int guildCount, int dmChannelCount)
    {
        ...
        _guildChannels = new ConcurrentDictionary<ulong, SocketChannel>(ConcurrentHashSet.DefaultConcurrencyLevel,
    ...
    {
        if (_guildChannels.TryRemove(id, out SocketChannel channel)) return channel;
    ...
    {
        foreach (SocketGuild guild in _guilds.Values) guild.PurgeChannelCache(this);
    }
    ...
    {
        if (_guilds.TryGetValue(id, out SocketGuild guild)) return guild;
    ...
    {
        if (_users.TryGetValue(id, out SocketGlobalUser user)) return user;

        return null;
    }

    internal SocketGlobalUser GetOrAddUser(ulong id, Func<ulong, SocketGlobalUser> userFactory) => _users.GetOrAdd(id, userFactory);

    internal SocketGlobalUser RemoveUser(ulong id)
    {
        if (_users.TryRemove(id, out SocketGlobalUser user)) return user;
    ...
    {
        foreach (SocketGuild guild in _guilds.Values) guild.PurgeUserCache();
    }
}
