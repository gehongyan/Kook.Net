...
public class ModuleBuilderTests
{
    ...
    {
        SectionModuleBuilder builder = new SectionModuleBuilder()
    ...
    {
        ImageGroupModuleBuilder builder = new ImageGroupModuleBuilder()
    ...
    {
        ContainerModuleBuilder builder = new ContainerModuleBuilder()
            .AddElement(b => b.WithSource(Icon));
    ...
    {
        ActionGroupModuleBuilder builder = new ActionGroupModuleBuilder()
    ...
    public void ActionGroupBuilder_ValidElements()
    {
        ...
        builder = new ActionGroupModuleBuilder { Elements = Enumerable.Repeat(new ButtonElementBuilder().WithText(Name), 4).ToList() };
        Assert.Equal(4, builder.Elements.Count);
    }
    ...
    {
        ContextModuleBuilder builder = new ContextModuleBuilder()
    ...
    {
        DividerModuleBuilder builder = new();
    ...
    {
        FileModuleBuilder builder = new FileModuleBuilder()
            .WithTitle(Name)
            .WithSource(Icon);
        ...
        builder = new FileModuleBuilder { Title = Name, Source = Icon };
        Assert.Equal(ModuleType.File, builder.Type);
        Assert.Equal(ModuleType.File, builder.Build().Type);
        Assert.Equal(Name, builder.Title);
        Assert.Equal(Name, builder.Build().Title);
    ...
    {
        VideoModuleBuilder builder = new VideoModuleBuilder()
            .WithTitle(Name)
            .WithSource(Icon);
        ...
        Assert.Equal(Icon, builder.Source);
        Assert.Equal(Icon, builder.Build().Source);
        builder = new VideoModuleBuilder { Title = Name, Source = Icon };
        Assert.Equal(ModuleType.Video, builder.Type);
        Assert.Equal(ModuleType.Video, builder.Build().Type);
        Assert.Equal(Name, builder.Title);
        Assert.Equal(Name, builder.Build().Title);
    ...
    {
        AudioModuleBuilder builder = new AudioModuleBuilder()
            .WithTitle(Name)
            .WithSource(Url)
            .WithCover(Icon);
        ...
        builder = new AudioModuleBuilder { Title = Name, Source = Url, Cover = Icon };
        Assert.Equal(ModuleType.Audio, builder.Type);
        Assert.Equal(ModuleType.Audio, builder.Build().Type);
        Assert.Equal(Name, builder.Title);
        Assert.Equal(Name, builder.Build().Title);
    ...
    public void CountdownModuleBuilder_InvalidTimes()
    {
        ...
        foreach ((CountdownMode countdownMode, DateTimeOffset endTime, DateTimeOffset? startTime) in GetInvalidTimes())
        {
            CountdownModuleBuilder builder = new CountdownModuleBuilder()
                ...
                .WithEndTime(endTime);
            if (startTime.HasValue) builder.WithStartTime(startTime.Value);
    ...
    {
        InviteModuleBuilder builder = new InviteModuleBuilder()
            .WithCode("abcdef");
        ...
        builder = new InviteModuleBuilder { Code = "ghijkl" };
        Assert.Equal(ModuleType.Invite, builder.Type);
        Assert.Equal(ModuleType.Invite, builder.Build().Type);
