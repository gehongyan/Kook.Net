...
- 得益于 KOOK 服务端的支持，`SocketGuildUser 的 `RoleIds` 与 `Roles` 属性现在会在为服务器用户授予或撤销角色时更新。
...
- 新增支持在修改消息时自动传递 `reply_msg_id` 参数，用于每日消息可发送条数总量消耗折扣。
...
- 修复 `KookDebuggers` 中注入的调试方法引发异常时导致应用程序崩溃的问题。

### 变更

- 移动配置项 `AutoLogoutBeforeLogin` 至 `KookSocketConfig` 中。
